...
export const creerPaiementSolde = (booking_id) => API.post(`/stripe/solde/${booking_id}`)
export const confirmerPaiementSolde = (booking_id, session_id) => API.put(`/stripe/solde/${booking_id}/confirmer`, { session_id })
export const getProfil = () => API.get('/profil')
export const modifierProfil = (data) => API.put('/profil', data)
export const modifierLanguePreferee = (langue_preferee) => API.put('/profil/langue', { langue_preferee })
...
export const telechargerExportComptable = async (date_debut, date_fin, langue) => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `export-comptable-${date_debut}-${date_fin}.pdf`
  document.body.appendChild(a)
  a.click()
  a.remove()
  window.URL.revokeObjectURL(url)
