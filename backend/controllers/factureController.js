...
const genererFacture = async (req, res) => {
  try {
    ...
    const { data: booking } = await supabase
      .from('bookings')
      .select('*, services(*, users(nom, prenom, email, adresse, siret)), users(nom, prenom, email, adresse)')
      .eq('id', booking_id)
      .single()
    ...
    doc.fillColor('#1A365D').fontSize(18).text('FACTURE', 400, 50, { align: 'right' })
    doc.fillColor('#3D2B0F').fontSize(10).text(`N° ${booking_id.slice(0, 8).toUpperCase()}`, 400, 75, { align: 'right' })
    doc.text(`Date : ${new Date(booking.date_rdv).toLocaleDateString('fr-FR')}`, 400, 90, { align: 'right' })

    doc.moveTo(50, 120).lineTo(550, 120).strokeColor('#A07840').stroke()

    doc.fillColor('#1A365D').fontSize(12).text('Prestataire', 50, 140)
    ...
    if (booking.services.users.siret) {
      doc.text(`SIRET : ${booking.services.users.siret}`, 50, 203)
    }

    doc.fillColor('#1A365D').fontSize(12).text('Client', 320, 140)
    doc.fillColor('#3D2B0F').fontSize(10)
    ...
    doc.fillColor('#1A365D').fontSize(14).text('Total TTC', 380, 340)
    doc.fillColor('#C53030').fontSize(16).text(`${booking.services.prix} €`, 480, 338)

    doc.fillColor('#A07840').fontSize(8).text(
      '© 2026 At Home Service — Cette facture est générée automatiquement.',
