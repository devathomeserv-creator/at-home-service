...
const creerService = async (req, res) => {
  try {
    const { categorie, titre, description, prix, duree, photo_url, pourcentage_acompte } = req.body
    const prestataire_id = req.user.id
    const { data, error } = await supabase
      .from('services')
      .insert([{ prestataire_id, categorie, titre, description, prix, duree, photo_url, pourcentage_acompte: pourcentage_acompte || 0 }])
      .select()
...
const modifierService = async (req, res) => {
  try {
    const { id } = req.params
    const { categorie, titre, description, prix, duree, disponible, photo_url, pourcentage_acompte } = req.body
    const { data, error } = await supabase
      .from('services')
      .update({ categorie, titre, description, prix, duree, disponible, photo_url, pourcentage_acompte: pourcentage_acompte || 0 })
      .eq('id', id)
